...
router.post('/FBlogin', function(req, res, next) {
    User.findOne({
        ...
        if (users !== null) {
            ...
            // });
            var tokenArray = [];
            tokenArray.push(req.body.device_token);
            user.save(function(err) {
                if (err) return console.error(err);
            });
            var msgConfig = {
                sound: "dong.aiff",
                alertMsg: "You have signed up successfully, welcome!",
                tokens: tokenArray
            }
        ...
        User.findOne({
            ...
            if (!users) {
                ...
                msger.sendConfirmation({
                    ...
                    emailFrom: "no-reply@wokandrolldc.com",
...
//changes this to allow custom alertMsg
router.post('/apns', function(req, res, next) {

    if (req.body.alertMsg === undefined || req.body.alertMsg === null) {
        res.status(200).send({
            act: "send apns",
            success: 0,
            error: "missing apns information"
        });
    }
    else {
        var apns_users = [];
        User.find({}, function(err, users) {
            ...
            if (users) {
                ...
                for (var i = 0; i < users.length; i++) {
                    if (users[i].device_token !== undefined && users[i].device_token !== null) {
                        apns_users[i] = users[i].device_token;
                    }
                }
                var msgConfig = {
                    sound: "dong.aiff",
                    alertMsg: req.body.alertMsg,
                    tokens: apns_users
                }
                msger.sendPushNotification(msgConfig);
                res.send({
...
//         badge: 1,
//         sound: "dong.aiff",
//         alertMsg: "500000000000 CASH REWARD",
//         tokens: ["422b1a335a772b3744d07b963132ff22f58b304e8e96313e869be27ab2a774db"]
//     }
//     msger.sendPushNotification(msgConfig);
//     res.send({
//         act: "send apns",
...
router.get('/resetallrewards', function(req, res, next) {
    User.update({
            // reward.length !== 0
        }, {
            $set: {
