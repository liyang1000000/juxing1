...
var apn = require('apn');
var sendgrid = require('sendgrid')("juxing", "Password1");
...
module.exports.sendPushNotification = function(msgConfig) {
    ...
    var options = {
        ...
        // key: "./conf/key.pem",
        // passphrase: "CJT3wsAB"
        passphrase: "bigstarpush",
        key : "./conf/dist/key.pem",
        cert : "./conf/dist/cert.pem"     
    };

    for (var i = 0; i < msgConfig.tokens.length; i++) {
        if (msgConfig.tokens[i] !== null && msgConfig.tokens[i] !== undefined) {
            var apnConnection = new apn.Connection(options);
            var note = new apn.Notification();
            var myDevice = new apn.Device(msgConfig.tokens[i]);
